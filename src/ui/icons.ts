// Inline SVG icons (Lucide, ISC licence). Rendered via `icon(name)`.
...
const paths = {
  ...
  taskList: '<rect width="18" height="18" x="3" y="3" rx="2"/><path d="m9 12 2 2 4-4"/>',
...
/** Fills every `[data-icon]` placeholder in the static HTML. */
export function hydrateIcons(root: ParentNode = document): void {
  root.querySelectorAll<HTMLElement>('[data-icon]').forEach((element) => {
    element.insertAdjacentHTML('afterbegin', icon(element.dataset.icon as IconName));
  });
}
